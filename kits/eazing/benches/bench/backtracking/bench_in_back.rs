...
pub fn in_back(c: &mut Criterion) {
  ...
  group.bench_function("eazing", |b| {
    ...
    b.iter(|| {
      let _ =
        black_box(nums.iter().map(|num| InBack.y(*num)).collect::<Vec<_>>());
    })
  });
    ...
    b.iter(|| {
      let _ = black_box(
        nums
          .iter()
          .map(|num| {
            bevy_tween::interpolation::EaseFunction::BackIn.sample(*num)
          })
          .collect::<Vec<_>>(),
      );
    })
  ...
  group.bench_function("easings", |b| {
    b.iter(|| {
      let _ = black_box(
        nums
          .iter()
          .map(|num| easings::back_in(*num as f64))
          .collect::<Vec<_>>(),
      );
    })
  });
  ...
  group.bench_function("emath", |b| {
    b.iter(|| {
      let _ = black_box(
        nums
          .iter()
          .map(|num: &f32| emath::easing::back_in(*num))
          .collect::<Vec<_>>(),
      );
    })
  });
  ...
  group.bench_function("interpolation", |b| {
    ...
    b.iter(|| {
      let _ =
        black_box(nums.iter().map(|num| num.back_in()).collect::<Vec<_>>());
    })
  });
  ...
  group.bench_function("simple_easing2", |b| {
    b.iter(|| {
      let _ = black_box(
        nums
          .iter()
          .map(|num| simple_easing2::back_in(*num))
          .collect::<Vec<_>>(),
      );
    })
  });
