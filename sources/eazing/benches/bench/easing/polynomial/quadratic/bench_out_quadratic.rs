    ...
    b.iter(|| {
      let _ = black_box(
        nums
          .iter()
          .map(|num| {
            bevy_tween::interpolation::EaseFunction::QuadraticOut.sample(*num)
          })
          .collect::<Vec<_>>(),
      );
    })
  ...
  group.bench_function("easings", |b| {
    b.iter(|| {
      let _ = black_box(
        nums
          .iter()
          .map(|num| easings::quadratic_out(*num as f64))
          .collect::<Vec<_>>(),
      );
    })
  });
  ...
  group.bench_function("emath", |b| {
    b.iter(|| {
      let _ = black_box(
        nums
          .iter()
          .map(|num: &f32| emath::easing::quadratic_out(*num))
          .collect::<Vec<_>>(),
      );
    })
  });
  ...
  group.bench_function("simple_easing2", |b| {
    b.iter(|| {
      let _ = black_box(
        nums
          .iter()
          .map(|num| simple_easing2::quad_out(*num))
          .collect::<Vec<_>>(),
      );
    })
  });
